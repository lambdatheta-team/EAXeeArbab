	...
	<form method="POST" action="DbToExcel" enctype="multipart/form-data">
		<h3>Add Data From Database To Excel</h3>
			<div class="form-group addAttribute">
				...
				<select class="form-control" type="text" name="db_Name" onchange="call( this.value); FILE_Relation(this.value); FILE_Relationship(this.value); " >
			...
			<h4 class="text-center text-uppercase">Relationship Data Data </h4>  
	        <div id="dataTable_wrapper"></div>  
	        <select id="tbl_id" name="tableName" class = "form-control" onchange="FILE_Relation(this.value)" >
	        
	        
	        <table id="Rel_id" class="table table-bordered table-striped" >  
	            <thead>  
	                <tr>  
	                    <th>Relationship</th>  
			...
			<h4 class="text-center text-uppercase">Relationship Table Data </h4>  
	        <div id="dataTable_wrapper"></div>  
	        <select id="tbl_id" name="tableName" class = "form-control" onchange="FILE_Relation(this.value)" >
	        
	        
	        <table id="Rel1_id" class="table table-bordered table-striped" >  
	            <thead>  
	                <tr>  
	                    <th>Name</th>  
...
function FILE_Relation (dbName){
	// alert('Receive');
	//let getdbName = document.getElementById("getDbName").value;
	console.log("dbName", dbName);
	$.ajax({
	      	url: "toRelationship", // url: "/findTables?dbName="+db_name,
		      //type: "GET",
			type: "POST",
			data: {
		    	  	'dbName' :dbName,
// 		    	  	'Source' :srcElement,
// 		    	  	'Destination' :tgtElement,
// 		    	  	'srcORtgtAtt' :srcORtgtAtt,
// 		    	  	'Relationship' :reltype,
			...
			success: function (data) {	
				...
				{	
					...
					RelationtName = data[i];
					console.log(" ", RelationtName)
					var x = document.getElementById("Rel_id");
					var td = document.createElement("td");
					td.text = data[i];
					x.append(td);
...
function FILE_Relationship (tableName){
	// alert('Receive');
	//let getdbName = document.getElementById("getDbName").value;
	console.log("tableName", tableName);
	$.ajax({
	      	url: "toAttribute", // url: "/findTables?dbName="+db_name,
		      //type: "GET",
			type: "POST",
			data: {
					'dbName' : dbName,
			...
			success: function (data) {	
				...
				{	
					...
					ElementName = data[i];
					console.log(" ", ElementName)
					var x = document.getElementById("Rel1_id");
					var td = document.createElement("td");
					td.text = data[i];
					x.append(td);
