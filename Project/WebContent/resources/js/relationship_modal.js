...
//			let table = $('#example').DataTable();
			...
			.node().id = idd+'_'+value.replaceAll(/[&\/\\#+()`$"\""~%'\'';,.:*?<>{}]/g, '_').replaceAll(" ", "_");
...
//		$.each( object_names, function( key, value ) {
		for(var i=0;i<loopLength;i++){
			...
			elementIcon =elementIconList[i].innerHTML;
			$(inputField).append('<tr><td class="elementIcon" style=\"width: 5px !important\">'+elementIcon+'</td><td class="object_name_form">'+tag_input+'</td></tr>');
...
function add_into_modal_table(tag_id,object_name, elementIcon,elementClass){
	...
	if(length_check > elementMaxNumber){
		...
		if($("#" + div_idd+'_'+tag_id.replaceAll(" ", "_")).length > 0 ) {
		  ...
		  //console.log("check", check)
			...
			$(".tree_modal_alert_message").html('<div class="alert alert-danger" role="alert"> '+Cant_add_more_than+' '+elementMaxNumber+'</div>');
		...
		if(!findObject){
			...
			let table = $('#example').DataTable();
			$('#example').DataTable().row.add([
			  '<td class="elementIcon"><img style=\"width:20px; height:14px\" draggable=\"false\" src=\"resources/js/examples/editors/images/images1/'+elementIcon+'\" ></td>', '<td class="object_name '+elementClass+'">'+object_name+'</td>', '<td><a style="cursor:pointer" onclick="removeRow(\''+div_idd+'_'+object_name.replaceAll(" ", "_")+'\')">&#10006;</a></td>'
			...
			.node().id = div_idd+'_'+tag_id.replaceAll(" ", "_");
			// $(t).addClass("object_name");
			table.draw();

			insert_class(div_idd+'_'+tag_id.replaceAll(" ", "_"),elementClass);
...
function removeRow(trId){
	...
	var rows = table
	    ...
	    $(".tree_modal_alert_message").html('<div class="alert alert-secondary" role="alert">'+remove_successfully+'</div>');
	    tree_modal_alert_message();
	// });
}
