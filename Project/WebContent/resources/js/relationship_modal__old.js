...
function relationship_modal(objectsArray,idd,maxPossibleObjects,elementIcon, save_btn_id){
	//$('#table_data').empty();
...
//		$.each( object_names_form, function( key, value ) {
		for(var i=0;i<loopLength;i++){
			...
			insert_class(idd+'_'+value.replaceAll(" ", "_"));
		}
...
//	createList(idd);
	//idds.push(idd);
	//}
	$('#addButtonRel').append('<button onclick="add_object(\''+idd+'\', \''+save_btn_id+'\');" class="form-control">Add</button>')
...
function add_into_modal_table(object_name, elementIcon){
	...
	if(length_check > max_number){
		...
		if(!findObject){
			...
			var table = $('#example').DataTable();
			$('#example').DataTable().row.add([
			  '<td class="elementIcon"><img style=\"width:20px; height:20px\" draggable=\"false\" src=\"resources/js/examples/editors/images/images1/'+elementIcon+'\" ></td>', '<td class="object_name">'+object_name+'</td>', '<td><a style="cursor:pointer" onclick="removeRow(\''+div_idd+'_'+object_name.replaceAll(" ", "_")+'\')">&#10006;</a></td>'
			...
			insert_class(div_idd+'_'+object_name.replaceAll(" ", "_"));
			// $('#example').dataTable( {
			//   "columnDefs": [
